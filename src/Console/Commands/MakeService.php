...
class MakeService extends Command
{
    ...
    {
        $name = str_replace(config('service-repository.service_interface_suffix'), '', $this->argument('name'));
    ...
    public function createService(string $className)
    {
        ...
        $serviceName = $nameOfService.config('service-repository.service_suffix');

        $namespace = $this->getNameSpace($className);
        $stubProperties = [
            '{namespace}' => $namespace,
            '{serviceName}' => $serviceName,
            '{serviceInterface}' => $nameOfService.config('service-repository.service_interface_suffix'),
    ...
    {
        $nameOfService = $this->getServiceName($className);
        $serviceName = $nameOfService.config('service-repository.service_interface_suffix');
    ...
    private function getServicePath($className, $servicename)
    {
        return $this->appPath().'/'.
            ...
            "/$className"."/$servicename".config('service-repository.service_suffix').'.php';
    }
    ...
    {
        return config('service-repository.repository_namespace')."\Eloquent";
    }
    ...
    {
        return $className.config('service-repository.repository_interface_suffix');
    }
    ...
    {
        return $className.config('service-repository.repository_suffix');
    }
    ...
    {
        $this->ensureDirectoryExists(config('service-repository.service_directory'));
        $this->ensureDirectoryExists(config('service-repository.service_directory').'/'.$className);
    }
    ...
    private function getNameSpace($className): string
    {
        ...
        if (count($explode) > 1) {
            ...
            return config('service-repository.service_namespace').$namespace.'\\'.end($explode);
        } else {
            return config('service-repository.service_namespace').'\\'.$className;
        }
    }
